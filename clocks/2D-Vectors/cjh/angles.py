...
from cjh.scalars import Scalar
...
class Angle(Scalar):
    ...
    def __init__(self, degrees=0.0, units_='deg', shell=cli.Cli):
        self.units = None #?
        super(Angle, self).__init__(degrees, units_)
        self.shell_obj = shell
        if units_ == '(pi) rad':
            self.units.abbrev = '\bπ rad'#fix this
    ...
    def __div__(self, divisor):
        ...
        quot = Angle(self.radians / decimal.Decimal(divisor), 'rad')
        if degrees_flag == True:
            self.deg()
            quot.deg()
        return quot
    ...
    def __str__(self):
        ...
        else: return '{} {}'.format(round(self.mag, 4), self.units.abbrev)
    ...
    def supplement(self):
        ...
        """
        return Angle(decimal.Decimal(180) - self.degrees, 'deg')
...
[{}]
      ...
      {:7g}° = {:6.4g} (i.e., {:5.5g}π) radian(s)
-------------------------------------------------
comp: {:7g}° = {:6.4g} (i.e., {:5.5g}π) radian(s)
supp: {:7g}° = {:6.4g} (i.e., {:5.5g}π) radian(s)
""".format(\
        ...
        lines = [
            line.rstrip().center(cli.Cli.width()) + '\n'\
