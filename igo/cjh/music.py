...
def get_scalar_freq(param):
    """
    Takes one number (Hz), or tuple (oct, local_halfsteps, cents);
    Returns a Scalar in Hz.
    """
    try:
...
def calc_halfsteps(param):
    """
    Takes one number (Hz), or a tuple (oct, local_halfsteps);
    Returns halfsteps above A440.
    """
    try:
        param = Decimal(param)
        halfsteps = 12.0 * math.log(param / Decimal(440.0)) / math.log(2.0)
    except ValueError:
        try:
            octave, note_float = param
            halfsteps = float(note_float) + (octave - 5) * 12.0
            num = int(note_float)
            if num in range(2):
                halfsteps += 12
        except ValueError:
...
def calc_local_semitones(halfsteps):
    """
    Takes halfsteps in relationship to A440;
    Returns the distance in semitones to the closest A, i.e.,
    'local_semitones'.
    """
...
class Pitch(SoundWave):
    ...
    def __init__(self, notename='C', octave=4.0, cents=0.0, freq=None):
        ...
        super(Pitch, self).__init__(self.freq.mag)
    ...
    def set_all_by_freq(self, freq):
        ...
        self.freq = get_scalar_freq(freq)
        self.halfsteps = calc_halfsteps(self.freq.mag)
        self.local_semitones = calc_local_semitones(self.halfsteps)
   ...
   #    else: return False
    ...
    def set_all_by_letter(self, octave, notename='C', cents=0.0):
        """
        Takes octave number and notename;
        Sets all other members based on this information.
        """
        ...
        self.halfsteps = calc_halfsteps((self.note_float, self.octave))
        note_tuple = (self.octave, self.note_number, self.cents)
        self.freq = get_scalar_freq(note_tuple)
        self.local_semitones = calc_local_semitones(self.halfsteps)
        self._finish_up()
    ...
    def letter2number(nname):
        ...
        if nname == 'A#' or nname == 'a#' or nname == 'Bb' or nname == 'bb' or\
            ...
            print(
                ...
                'Invalid notename.')  # pylint: disable=C0325
            number = -1
        return number
    ...
    def set_oct(freq):
        ...
        freq = Decimal(freq)
        oct_ = int(math.log((freq / Decimal(15.886)), 2))
        return oct_
...
class Note(Pitch):
    ...
    def __str__(self):
        string = '\n{}\n'.format(Cli.term_fx('ul', self.label))
        string += '{}\n'.format(self.freq)
        string += self.halfsteps_info()
        return string
