...
class Polynom(Thing, Minusable):
    ...
    def __add__(self, other):
        ...
        try:
            for term in other.list_:
                if term.exp in list(sum_.dict.keys()):
            ...
            # if addend is Term
            try:
                if other.exp in list(sum_.dict.keys()):
    ...
    def __mul__(self, other):
        ...
        poly.dict = {coef_: (decimal.Decimal(other) * self.dict[coef_])
            for coef_ in list(self.dict.keys())}
        ...
        for exp in range(int(max(poly.dict.keys())), -1, -1):
            try:
                if poly.dict[exp] == 0.0:
                    ...
                    poly.list_.append(Term(poly.dict[exp], exp))
            except KeyError:
