...
class TkTemplate(WindowedApp):
    ...
    def add_message_widget(cls):
        ...
        cls.msg = tkinter.Message(cls.main_window, textvariable=cls.msgtxt, width=200)
    ...
    def input(cls, prompt='Enter something:'):
        ...
        """
        input_dialog = TkInput(cls.main_window, prompt)
...
class TkDialog(TkPopup):
    ...
    def __init__(
        ...
        super(TkPopup, self).__init__(parent, msg, heading, width, height)
...
class TkList(object):
    ...
    def __init__(
        ...
        top.title(title)
        height = (len(list_obj) + 3) * 16 + 8
        TkTemplate.center_window(
            win=top, x_offset=0, height_=height, width_=150, y_offset=-30)
        tkinter.Label(top, text=prompt).pack()
        #if len(list_obj) < 12:
        tall = len(list_obj)
        #else: tall = 12
        self.listb = tkinter.Listbox(top, height=tall, width=17)
